...
func (c *Community) MemberCount(ctx context.Context, hidden bool) (int, error) {
	...
	if hidden {
		...
		err = amdb.GetContext(ctx, &rc, "SELECT COUNT(*) FROM commmember WHERE commid = ? AND hidden = 0", c.Id)
	}
	if err == nil {
		return rc, nil
	}
...
func (c *Community) ListMembers(ctx context.Context, field int, oper int, term string, offset int, max int, showHidden bool) ([]*User, int, error) {
	...
	var err error
	var rs *sql.Rows
	if err = amdb.GetContext(ctx, &total, `SELECT COUNT(*) FROM commmember m, users u, contacts c WHERE m.commid = ? AND m.uid = u.uid
...
func (c *Community) SetProfileData(ctx context.Context, name string, alias string, synopsis *string, rules *string, language *string,
	...
	if err == nil {
		...
		err2 := amdb.GetContext(ctx, &(c.LastUpdate), "SELECT lastupdate FROM communities WHERE commid = ?", c.Id)
		if err2 != nil {
...
func (c *Community) Touch(ctx context.Context) error {
	...
	if err == nil {
		var na time.Time
		if err = amdb.GetContext(ctx, &na, "SELECT lastaccess FROM communities WHERE commid = ?", c.Id); err == nil {
			c.LastAccess = &na
		}
	}
	return err
...
func AmCreateCommunity(ctx context.Context, name string, alias string, hostUid int32, language *string, synopsis *string,
	...
	var tmpcid int32
	if err := tx.GetContext(ctx, &tmpcid, "SELECT commid FROM communities WHERE alias = ?", alias); err != sql.ErrNoRows {
		if err == nil {
...
func AmSearchCommunities(ctx context.Context, field int, oper int, term string, offset int, max int, showAll bool) ([]*Community, int, error) {
	...
	var total int
	err := amdb.GetContext(ctx, &total, "SELECT COUNT(*) FROM communities "+q)
	if err != nil || total == 0 {
		return make([]*Community, 0), 0, err // short-circuit return
	}
	var rs *sql.Rows
...
func AmSearchCommunityMembers(ctx context.Context, c *Community, field int, oper int, term string, offset int, max int) ([]*User, int, error) {
	...
	q := queryPortion.String()
	var total int
	err := amdb.GetContext(ctx, &total, `SELECT COUNT(*) FROM users u, contacts c, commmember m WHERE u.contactid = c.contactid AND u.uid = m.uid
